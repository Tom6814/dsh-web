...
if [ "$DSH_PROFILE" = "web" ]; then
  ...
  if [ ! -d "$DSH_HOME/profiles/web/node_modules/dsh-automation" ]; then
    echo "    automation: 首次安装…"
    dsh plugin --profile web add /opt/dsh-zeabur/plugins/automation
  fi
  if [ ! -d "$DSH_HOME/profiles/web/node_modules/dsh-image-gen" ]; then
    echo "    image-gen: 首次安装…"
    dsh plugin --profile web add /opt/dsh-zeabur/plugins/image-gen
  fi
  if [ ! -d "$DSH_HOME/profiles/web/node_modules/dsh-model-extras" ]; then
