// dsh-super-injector client（由 tsc 产物转换，loader 格式）
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// 设置页 UI 已移除：它注册的 settings.section（label「插件」）与官方插件区重复，
		// 且其 render() 返回 { dispose } 而非 DOM 导致展开为空。核心能力（dev_* 工具）在 host 端不受影响。
		function apply(_ctx) {
		    /* 有意为空：不再注册 settings.section UI */
		}
